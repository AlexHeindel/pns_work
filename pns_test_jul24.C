	...
	*/
	...
	for(int i = 0; i<11; i++)
	{
		...
		harray[i]->GetXaxis()->SetTitle("\\mbox{Time} (\\mu s)");
        	harray[i]->Draw();
        	TString filename = Form("images_jul24/timeine%d_test.png",int_array[i]);
		c->SaveAs(filename);	
	}
	...
	//hs->GetXaxis()->SetTitle("\\mbox{Time} (\\mu s)");
        //hs->Draw();
        //timehstack->Draw();
        //cs->SaveAs("images_jul24/timeinestack_test.png");
	/*
	TCanvas *c6 = new TCanvas();
	timeine303->GetXaxis()->SetTitle("\\mbox{Time} (\\mu s)");
        timeine303->Draw();
        c6->SaveAs("images_jul24/timeine303_new.png");
	
	TCanvas *c7 = new TCanvas();
	timeine306->GetXaxis()->SetTitle("\\mbox{Time} (\\mu s)");
        timeine306->Draw();
        c7->SaveAs("images_jul24/timeine306_new.png");
	*/
	//timehstack->Add(timeine303);
	//timehstack->Add(timeine306);

	//TCanvas *c8 = new TCanvas();
	//timehstack->GetXaxis()->SetTitle("\\mbox{Time} (\\mu s)");
        //timehstack->Draw();
        //c8->SaveAs("images_jul24/timeinestack_new.png");
}
